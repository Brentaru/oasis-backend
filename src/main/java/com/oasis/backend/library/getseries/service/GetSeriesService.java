package com.oasis.backend.library.getseries.service;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.oasis.backend.library.getseries.dto.GetSeriesResponse;
...
public class GetSeriesService {
    ...
    public ResponseEntity<?> getSeries() {
        ...
        try {
            ...
            List<GetSeriesResponse> response = rows.stream()
                    .map(row -> new GetSeriesResponse(
                            value(row.get("id")),
                            value(row.get("title")),
            ...
            return ResponseEntity.status(e.getStatusCode()).body("Failed to get series.");

        } catch (RestClientResponseException e) {
            return ResponseEntity.status(400).body("Failed to get series.");
