package com.oasis.backend.browse.filterseries.service;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.oasis.backend.browse.filterseries.dto.FilterSeriesResponse;
...
@Service
public class FilterSeriesService {
    ...
    public ResponseEntity<?> filterSeries(String genre, String status, String keyword) {

        String url = supabaseUrl + "/rest/v1/" + seriesTable
                + "?select=id,title,author,cover_image,description,genre,status,rating";

        if (genre != null && !genre.isBlank()) {
            url += "&genre=eq." + encode(genre.trim());
        }

        if (status != null && !status.isBlank()) {
            url += "&status=eq." + encode(status.trim());
        }

        if (keyword != null && !keyword.isBlank()) {
            String likeValue = encode("*" + keyword.trim() + "*");
            url += "&or=(title.ilike." + likeValue
        ...
        try {
            ...
            List<FilterSeriesResponse> response = rows.stream()
                    .map(row -> new FilterSeriesResponse(
                            value(row.get("id")),
                            value(row.get("title")),
            ...
            return ResponseEntity.status(e.getStatusCode()).body("Failed to filter series.");

        } catch (RestClientResponseException e) {
            return ResponseEntity.status(400).body("Failed to filter series.");
